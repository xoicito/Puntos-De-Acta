...
def generate_acta(item_id):
    item=get_item(item_id); data=item_data(item)
    ...
    try:
        ...
        replacements={
          "{{PROYECTO}}":data["proyecto"], "{{RUBRO}}":data["rubro"], "{{NO_CONTRATO}}":data["no_contrato"],
          "{{TIPO_CONTRATO}}":data["tipo_contrato"], "{{FECHA_ACTA}}":display_date(data["fecha_acta"]), "{{EMPRESA}}":data["empresa"],
          "{{CONTACTO}}":data["contacto"], "{{TELEFONO}}":data["telefono"], "{{NIT}}":data["nit"], "{{NO_COTIZACION}}":data["no_cotizacion"],
          "{{ANTICIPO}}":pct(data["anticipo"]), "{{ESTIMACIONES}}":pct(data["estimaciones"]), "{{CONTRA_ENTREGA}}":pct(data["contra_entrega"]), "{{RETENIDO}}":pct(data["retenido"]),
        }
        replacements.update(build_blocks(data,rubrics))
        missing=[k for k in ("proyecto","rubro","no_contrato","empresa") if not data.get(k)]
...
if __name__=="__main__":
    import argparse
    p=argparse.ArgumentParser(); p.add_argument("item_id"); a=p.parse_args(); print(generate_acta(a.item_id))
