...
def _notify_pmo(item_id, board_id, item):
    """The PMO the Lider picked (same "Gerentes" board, just a different
    item) doesn't sign anything - it just gets an email once the Gerente
    signs. Resolves the PMO's email and writes it into PMO_EMAIL_COLUMN_ID;
    a Monday automation (set up in the UI) sends the actual email to that
    address, same pattern as the Gerente's signing-link email.
    """
